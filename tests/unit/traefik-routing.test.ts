...
describe("TraefikService.setLoadingMode", () => {
  it("points service URL at loading stub", async () => {
    const redis = makeRedis();
  ...
  it("returns 200 path — stub address is the nginx loading server, not a dead port", async () => {
    const redis = makeRedis();
    await makeTraefik(redis).setLoadingMode("api");
    assert.equal(redis.store.get(KV_KEYS.TRAEFIK.SERVICE_URL("api")), SERVICE_STUB_ADDRESS);
  });
});
...
describe("TraefikService.unregisterRoute", () => {
  it("removes all route keys", async () => {
    ...
    await svc.registerRoute({ serviceName: "api", instanceAddress: "10.0.0.1" });
...
describe("TraefikService.getRouteBackendUrl", () => {
  it("returns null when service is not registered", async () => {
    const redis = makeRedis();
    assert.equal(await makeTraefik(redis).getRouteBackendUrl("unknown"), null);
  });
