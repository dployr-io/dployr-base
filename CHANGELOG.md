...
- *(admin)* Implement administrative API and JWT management
- *(config)* Integrate admin security settings into environment bindings
- *(ui)* Add static assets and base index page
- *(errors)* Implement centralized error handling system
- *(instances)* Modularize instance route handlers and helpers to reduce repitittion accross user and admin instance handlers
- *(auth)* Persist user clusters on successful oauth callback
- *(server)* Add static asset routes for icon and favicon

### 💼 Other

- *(deps)* Add otpauth dependency

### 🚜 Refactor

- *(db)* Migrate primary key types from text to uuid
- *(config)* Simplify schema and add admin configuration
- *(constants)* Modularize constants and update admin configuration
- *(types)* Update environment bindings for admin security
...
- *(middleware)* Add JSDoc documentation for security layers
- Update installation instructions in README

### 🎨 Styling

- *(proxy)* Fix formatting and trailing commas
- *(utils)* Reformat code for consistency

### ⚙️ Miscellaneous Tasks

- *(config)* Ignore tsbuildinfo files
- *(scripts)* Add totp setup utility
- *(db)* Standardize error handling and key constants
- *(install)* Refactor installation script and add config processor
## [0.2.0] - 2026-04-19

### 🚀 Features

- *(tasks)* Add type and runtime validation to deployment schema

### 🚜 Refactor

- *(node)* Rename agent to node and implement node update processing
## [0.1.50] - 2026-01-17
