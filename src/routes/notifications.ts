...
notifications.post("/events/setup", requireClusterDeveloper, async (c) => {
  try {
    ...
    log.error("Notification events setup error:", error);
    return c.json(
      createErrorResponse({
...
notifications.post("/discord/setup", requireClusterDeveloper, async (c) => {
  try {
    ...
    log.error("Discord setup error:", error);
    return c.json(
      createErrorResponse({
        message: "Internal server error",
        code: ERROR.RUNTIME.INTERNAL_SERVER_ERROR.code,
...
notifications.post("/slack/setup", requireClusterDeveloper, async (c) => {
  try {
    ...
    log.error("Slack setup error:", error);
    return c.json(
      createErrorResponse({
        message: "Internal server error",
        code: ERROR.RUNTIME.INTERNAL_SERVER_ERROR.code,
...
notifications.post("/email/setup", requireClusterDeveloper, async (c) => {
  try {
    ...
    log.error("Email notification setup error:", error);
    return c.json(
      createErrorResponse({
        message: "Internal server error",
        code: ERROR.RUNTIME.INTERNAL_SERVER_ERROR.code,
