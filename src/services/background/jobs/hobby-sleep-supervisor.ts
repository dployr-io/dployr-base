...
export const hobbySleepSupervisor: JobFn = async ({ db, kv, adapters }) => {
  ...
  for (const service of services) {
    ...
    const sleeping = await kv.kv.get(KV_KEYS.SERVICE.SLEEPING(service.name));
    ...
    // Find the routing key for this cluster's instance
    ...
    const taskId = ulid();
    const token = await jwtService.createNodeAccessToken(routingKey, { issuer: adapters.config.server.base_url, audience: "dployr-instance" });
