...
/**
 * WebSocket message rate limit (for DO to enforce)
 */
export async function checkWebSocketMessageRateLimit(
  kv: KVNamespace,
  identifier: string,
  windowMs: number = 1000,
  maxMessages: number = 50
): Promise<{ allowed: boolean; retryAfter?: number }> {
  const key = `ratelimit:ws:${identifier}`;
  const now = Date.now();
  ...
  try {
    ...
    console.error("WS rate limit check failed:", error);
