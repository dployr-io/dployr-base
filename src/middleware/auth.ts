 ...
 */
export function resolveCluster(entity: "instance" | "domain" | "service" | "deployment" | "proxy", options: { path?: string; body?: string; query?: string; lookupBy?: "id" | "tag" }) {
  return async (c: Context<{ Bindings: Bindings; Variables: Variables }>, next: Next) => {
    ...
    if (entity === "instance") {
      ...
      if (!deployment) {
        return c.json({ error: "Deployment not found", code: ERROR.RESOURCE.MISSING_RESOURCE.code }, ERROR.RESOURCE.MISSING_RESOURCE.status);
      }
